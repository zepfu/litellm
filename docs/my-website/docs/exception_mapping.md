...
| 400         | RejectedRequestError | litellm.BadRequestError | Raised when a proxy guardrail or rule rejects the request |
...
| 502 | BadGatewayError | openai.APIStatusError | If a provider returns a bad gateway error, this error is raised |
