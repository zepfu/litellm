...
| OpenRouter | model names starting with `openrouter/` are routed to OpenRouter; model-specific capability support is metadata-backed | [OpenRouter model metadata snapshot](../providers/openrouter-model-metadata)
