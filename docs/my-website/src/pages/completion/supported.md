...
LiteLLM supports explicit `openrouter/<model-id>` routing, including model IDs
not present in LiteLLM's metadata. For model-specific capability coverage, see
the [OpenRouter model metadata snapshot](/docs/providers/openrouter-model-metadata).
