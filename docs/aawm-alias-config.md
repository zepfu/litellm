...
### Credential cooldown scope

Direct Cohere alias cooldowns keep three explicit scopes. Authentication
(HTTP 401/403), billing exhaustion (HTTP 402), and monthly quota failures
...
per-minute rate limit stays on the model key. Exhausting an explicitly
per-minute quota while naming that plan, or beside a separate monthly-usage
figure, also stays on the model key. Credential scope for HTTP 429 requires
exhaustion of the monthly quota or capacity itself.
