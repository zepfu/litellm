...
def get_llm_provider(  # noqa: PLR0915
    ...
    try:
        ...
        if custom_llm_provider == "openrouter" and "/" in model:
            return model, custom_llm_provider, dynamic_api_key, api_base
        ...
        if api_base:
            for endpoint in litellm.openai_compatible_endpoints:
                if endpoint in api_base:
                    ...
                    elif endpoint == "https://inference-api.nousresearch.com/v1":
...
def _get_openai_compatible_provider_info(  # noqa: PLR0915
    ...
    if custom_llm_provider == "perplexity":
        ...
        (
            api_base,
            dynamic_api_key,
        ) = litellm.AlibabaTokenPlanChatConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
    ...
    # publicai is now handled by JSON config (see litellm/llms/openai_like/providers.json)
    ...
    elif custom_llm_provider == "nous":
        (
            api_base,
            dynamic_api_key,
        ) = litellm.NousChatConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            api_base,
            dynamic_api_key,
        ) = litellm.CursorAgentConfig()._get_provider_info(api_base, api_key)
