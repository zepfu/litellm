...
def credential_identity(
    ...
    *,
    ...
    The identity deliberately excludes access/refresh/id tokens and raw
    filesystem paths. A hash of the canonical auth-file path, file metadata,
    and nonsecret lifecycle fields distinguishes copied files and atomic
    generations while remaining safe for scheduler state and telemetry.
    """

    auth_path_hash = auth_path_identity_hash(auth_path)
    ...
    if (
        auth_path_hash is None
        and not stat_parts
        and not safe_record
        and _clean_string(scope) is None
    ):
        return None
    payload = {
        "auth_path_hash": auth_path_hash,
...
def auth_path_identity_hash(auth_path: Optional[AuthPathValue]) -> Optional[str]:
    """Hash the canonical path for telemetry without returning the path."""

    value = _clean_string(auth_path)
    if value is None:
        return None
    canonical_path = Path(value).expanduser().resolve(strict=False)
