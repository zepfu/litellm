...
from .policy import (
    ...
    CODEX_AUTO_AGENT_NOUS_LANE_KEY as _CODEX_AUTO_AGENT_NOUS_LANE_KEY,
    CODEX_AUTO_AGENT_NOUS_PROVIDER as _CODEX_AUTO_AGENT_NOUS_PROVIDER,
    CODEX_AUTO_AGENT_CURSOR_AGENT_LANE_KEY as _CODEX_AUTO_AGENT_CURSOR_AGENT_LANE_KEY,
    CODEX_AUTO_AGENT_CURSOR_AGENT_PROVIDER as _CODEX_AUTO_AGENT_CURSOR_AGENT_PROVIDER,
    CODEX_AUTO_AGENT_NVIDIA_LANE_KEY as _CODEX_AUTO_AGENT_NVIDIA_LANE_KEY,
...
def _resolve_codex_request_mode_and_ordinal(
    *,
    ...
    request_body: dict[str, Any],
    server_validated_replay: bool = False,
) -> tuple[str, Optional[int]]:
    redispatch_ordinal = _extract_codex_request_redispatch_ordinal(request_body)
    # The ordinal is client-visible telemetry only. A fresh redispatch mode is
    # authorized solely by server-owned validation bound to the exact rebuilt
    # replay body.
    if server_validated_replay:
...
def _build_auto_agent_terminal_candidate_inventory(  # noqa: PLR0915
    *,
    ...
    def _occurrence_discriminator(
        ...
        """Identify one compiled route occurrence without account state.
    ...
    def _capture_identity(candidate: Mapping[str, Any]) -> tuple[Any, ...]:
        """Identify one route occurrence plus its account/lane/cooldown."""

        return (
            _occurrence_discriminator(candidate),
            ...
            _first_identity_value(candidate, "cooldown_scope"),
        )

    def _skip_identity(candidate: Mapping[str, Any]) -> tuple[Any, ...]:
        return (
            *_capture_identity(candidate),
            _first_identity_value(candidate, "skip_reason", "reason")
            or "unavailable",
            _first_identity_value(
                candidate,
                "candidate_semantic_ineligibility_reason",
    ...
    # Captured records are authoritative for an occurrence.  Group provider
    # attempts by exact account/lane/cooldown identity so failover records do
    # not overwrite one another, while preserving the provider-attempt count
    # within each captured occurrence.
    provider_attempt_groups: dict[
        tuple[Any, ...], dict[str, Any]
    ] = {}
    terminal_skip_groups: dict[tuple[Any, ...], dict[str, Any]] = {}
    for attempt_index, attempt in enumerate(normalized_attempts):
        if attempt.get("terminal_disposition") == "skipped":
            group_identity = _skip_identity(attempt)
            groups = terminal_skip_groups
        else:
            group_identity = _capture_identity(attempt)
            groups = provider_attempt_groups
        group = groups.get(group_identity)
        if group is None:
            group = {
                "occurrence": _occurrence_discriminator(attempt),
                "first_index": attempt_index,
                "records": [],
            }
            groups[group_identity] = group
        group["records"].append(attempt)
    ...
    def _apply_skip_overlay(
        ...
        shaped["reason"] = (
            ...
            or skipped.get("reason")
            or "unavailable"
        )
        return shaped

    def _shape_captured_attempt(
        ...
        for field in skip_metadata_fields:
            if field in attempt:
                shaped[field] = attempt[field]
        return shaped

    def _render_provider_attempt_group(
        group: Mapping[str, Any],
    ) -> dict[str, Any]:
        records = group["records"]
        last_attempt = records[-1]
        shaped = _shape_captured_attempt(last_attempt)
        shaped.update(
            {
                "terminal_disposition": "attempted",
                "attempt_count": _provider_attempt_count(records),
                "outcome": (
    ...
    def _render_terminal_skip_group(
        group: Mapping[str, Any],
    ) -> dict[str, Any]:
        shaped = _shape_captured_attempt(group["records"][-1])
        return _apply_skip_overlay(shaped, group["records"][-1])

    captured_groups: list[dict[str, Any]] = [
        *provider_attempt_groups.values(),
        *terminal_skip_groups.values(),
    ]
    terminal_skip_identities = set(terminal_skip_groups)
    for skipped_index, skipped_candidate in enumerate(normalized_skipped):
        occurrence_identity = _skip_identity(skipped_candidate)
        if occurrence_identity in terminal_skip_identities:
            continue
        captured_groups.append(
            {
                "occurrence": _occurrence_discriminator(skipped_candidate),
                "first_index": len(normalized_attempts) + skipped_index,
                "records": [skipped_candidate],
                "kind": "skipped_candidate",
            }
        )
    for group in provider_attempt_groups.values():
        group["kind"] = "provider_attempt"
    for group in terminal_skip_groups.values():
        group["kind"] = "terminal_skip"
    captured_groups.sort(key=lambda group: int(group["first_index"]))
    captured_groups_by_occurrence: dict[
        tuple[Any, ...], list[dict[str, Any]]
    ] = {}
    for group in captured_groups:
        captured_groups_by_occurrence.setdefault(
            group["occurrence"], []
        ).append(group)
    ...
    for candidate in compiled_candidates:
        occurrence = _occurrence_discriminator(candidate)
        captured_for_occurrence = captured_groups_by_occurrence.get(occurrence)
        if captured_for_occurrence:
            for group in captured_for_occurrence:
                group_id = id(group)
                if group_id in consumed_group_ids:
                    continue
                consumed_group_ids.add(group_id)
                if group["kind"] == "provider_attempt":
                    inventory.append(_render_provider_attempt_group(group))
                elif group["kind"] == "terminal_skip":
                    inventory.append(_render_terminal_skip_group(group))
                else:
                    inventory.append(
                        _apply_skip_overlay(
                            _shape_captured_attempt(group["records"][-1]),
                            group["records"][-1],
                        )
                    )
            continue
    ...
    for group in captured_groups:
        if id(group) in consumed_group_ids:
            continue
        consumed_group_ids.add(id(group))
        if group["kind"] == "provider_attempt":
            inventory.append(_render_provider_attempt_group(group))
        elif group["kind"] == "terminal_skip":
            inventory.append(_render_terminal_skip_group(group))
...
async def _apply_cohere_credential_lane_cooldown(
    *,
    candidate: dict[str, Any],
    cooldown_seconds: float,
    cooldown_state_source: Optional[str],
    ...
    key = cohere_credential_lane_cooldown_key(candidate, sentinel)
    if key is None:
        return cooldown_seconds, cooldown_state_source, skip_reason, None
...
async def _build_codex_auto_agent_candidate_state(  # noqa: PLR0915
    ...
    *,
    ...
    elif candidate["provider"] == _CODEX_AUTO_AGENT_ZAI_CODING_PLAN_PROVIDER:
        lane_key = _CODEX_AUTO_AGENT_ZAI_CODING_PLAN_LANE_KEY
    elif candidate["provider"] == _CODEX_AUTO_AGENT_COHERE_PROVIDER:
        lane_key = _CODEX_AUTO_AGENT_COHERE_LANE_KEY
    elif candidate["provider"] == _CODEX_AUTO_AGENT_NOUS_PROVIDER:
        lane_key = _CODEX_AUTO_AGENT_NOUS_LANE_KEY
    elif candidate["provider"] == _CODEX_AUTO_AGENT_CURSOR_AGENT_PROVIDER:
        lane_key = _CODEX_AUTO_AGENT_CURSOR_AGENT_LANE_KEY
    elif candidate["provider"] == _CODEX_AUTO_AGENT_NVIDIA_PROVIDER:
        lane_key = _CODEX_AUTO_AGENT_NVIDIA_LANE_KEY
    elif candidate["provider"] == _CODEX_AUTO_AGENT_OPENCODE_PROVIDER:
        lane_key = _CODEX_AUTO_AGENT_OPENCODE_LANE_KEY
    elif candidate["provider"] == _CODEX_AUTO_AGENT_OPENCODE_GO_PROVIDER:
        lane_key = _CODEX_AUTO_AGENT_OPENCODE_GO_LANE_KEY
    else:
    ...
    (
        ...
        cooldown_state_source,
        skip_reason,
        cohere_credential_cooldown_scope,
    ) = await _apply_cohere_credential_lane_cooldown(
        candidate=candidate,
        cooldown_seconds=cooldown_seconds,
        cooldown_state_source=cooldown_state_source,
        skip_reason=skip_reason,
...
def _raise_codex_auto_agent_authenticated_continuation_unavailable(
    *,
    ...
    retry_after_seconds: Optional[Any] = None,
    error_tokens: Optional[set[str]] = None,
    error_status_code: Optional[Any] = None,
    error_type: Optional[str] = None,
    error_code: Optional[Any] = None,
    attempted_provider_call: Optional[bool] = None,
    ...
    detail = _build_auto_agent_redispatch_http_exception_detail(
        ...
        lane_key=lane_key,
        cooldown_seconds=cooldown_seconds,
        error_tokens=error_tokens if error_tokens is not None else set(),
        error_class=error_class,
        cooldown_scope=cooldown_scope,
...
async def _select_codex_auto_agent_candidate(  # noqa: PLR0915
    *,
    ...
    server_validated_replay = sa.validate_cursor_replay_matches_body(
        request,
        body=request_body,
    )
    request_mode, redispatch_ordinal = _resolve_codex_request_mode_and_ordinal(
        ...
        server_validated_replay=server_validated_replay,
    )
    ...
    async def _reselect_owned_affinity_with_effective_identity(
        *,
        ...
        lease_cleared = sa.clear_expected_non_held_request_session_owner_lease(
            request,
        ...
        if not lease_cleared:
            sa.raise_session_owner_redispatch_required(
                ...
                guard=sa.SessionOwnerGuardResult(
                    ...
                    mismatch_reason=(
                        "session_owner: held lease cannot transition to derived identity"
                    ),
                ),
                request=request,
            )
    ...
    if isinstance(session_owner_record, dict) and sa._record_state(session_owner_record) == "owned":
        ...
        if (
            not is_auto_review
            and sa._hosted_provider_from_attributes(
                sa._owner_attributes(session_owner_record)
            )
            == "cursor"
        ):
            cursor_effective_identity = (
                session_owner_identity
                if sa.is_session_owner_redispatch_effective_identity(
                    session_owner_identity
                )
                else sa.derive_session_owner_effective_identity(
                    session_owner_identity
                )
            )
            if cursor_effective_identity is None:
                sa.raise_session_owner_redispatch_required(
                    session_identity=session_owner_identity,
                    alias_model=alias_model,
                    ...
                    guard=sa.SessionOwnerGuardResult(
                        ...
                        owner_record=session_owner_record,
                        owner_id=session_owner_record.get("owner"),
                        mismatch_reason="session_owner: Cursor rediscovery identity unavailable",
                    ),
                    request=request,
                )
            cursor_effective_record, cursor_effective_cache_key, cursor_effective_error = (
                await sa.get_session_owner_record(
            ...
            if cursor_effective_error is not None:
                sa.raise_session_owner_redispatch_required(
                    ...
                    guard=sa.SessionOwnerGuardResult(
                        decision=sa.SessionOwnerGuardDecision.REDISPATCH_REQUIRED,
                        session_identity=cursor_effective_identity,
                        cache_key=cursor_effective_cache_key,
                        owner_record=cursor_effective_record,
                        owner_id=(
                            cursor_effective_record.get("owner")
                            if isinstance(cursor_effective_record, dict)
                            else None
                        ),
                        mismatch_reason=cursor_effective_error,
                    ),
                    request=request,
                )
            if cursor_effective_record is None:
                # The base Cursor owner remains authoritative for the initial
                # request. Do not activate an unproven derived identity.
                if sa.is_session_owner_redispatch_effective_identity(
                    session_owner_identity
                ):
                    sa.raise_session_owner_redispatch_required(
                        ...
                        guard=sa.SessionOwnerGuardResult(
                            ...
                            owner_id=session_owner_record.get("owner"),
                            mismatch_reason=(
                                "session_owner: derived Cursor owner record unavailable"
                            ),
                        ),
                        request=request,
                    )
            ...
            elif sa._record_state(cursor_effective_record) == "owned":
                if cursor_effective_identity != session_owner_identity:
                    activated_identity = (
                    ...
                    if activated_identity != cursor_effective_identity:
                        sa.raise_session_owner_redispatch_required(
                            session_identity=session_owner_identity,
                            alias_model=alias_model,
                            failure_phase="session_owner_effective_identity_conflict",
                            guard=sa.SessionOwnerGuardResult(
                                ...
                                owner_id=session_owner_record.get("owner"),
                                mismatch_reason=(
                                    "session_owner: Cursor derived identity could "
                                    "not be activated"
                                ),
                            ),
                            request=request,
                ...
                sa.raise_session_owner_redispatch_required(
                    ...
                    guard=sa.SessionOwnerGuardResult(
                        ...
                        mismatch_reason=(
                            "session_owner: Cursor derived owner is not owned"
                        ),
                    ),
                    request=request,
                )
    ...
    states = await _build_codex_auto_agent_candidate_states(
        ...
        include_candidate_semantic_ineligibility=(
            _candidate_semantic_marker_consult_eligible(
                request_mode=request_mode,
                has_continuation_state=provider_owned_continuation,
                has_previous_response_id=has_previous_response_id,
                has_account_bound_state=effective_account_bound_state,
