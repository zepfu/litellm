...
Hidden-retry metadata derives the logical retry count from admitted ledger
reservations for the operation minus its initial send. A denied reservation is
recorded separately as `reservation_denied` and cannot become a logical retry
or provider-send event. Transport connection failures remain separate
evidence.
