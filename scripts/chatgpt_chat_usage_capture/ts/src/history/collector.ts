...
export class HistoryCollector {
  ...
  private async fetchModernMessages(
    ...
    while (detailPagesFetched < pageBudget) {
      ...
      if (page.paginationState === "contradictory") {
        this.markMalformedPage(currentRevisit, "contradictory_pagination", page.warnings);
      } else if (page.coverage === "unrecognized") {
        this.markMalformedPage(currentRevisit, "unrecognized_detail", page.warnings);
      } else if (page.coverage === "partial" || page.warnings.length > 0) {
        this.markMalformedPage(currentRevisit, "partial_detail", page.warnings);
      }
    ...
    const entry: RevisitEntry = {
      ...
      outstandingGeneration:
        reason === "nonterminal_generation"
          ? { state: "unknown", since: nowIso, timedOut: false }
