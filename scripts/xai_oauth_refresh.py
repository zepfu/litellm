...
def inspect_xai_oauth_credential_health(
    ...
    *,
    ...
    resolved_scope = _fallback_scope(scope)
    try:
        ...
        access_available = _credential_access_token(credential) is not None
        ...
        if not access_available:
            return _xai_health_summary(
                ...
                error_message="xAI OAuth credential does not contain an access credential.",
        ...
        if now >= expires_at - timedelta(seconds=resolved_buffer_seconds):
            return _xai_health_summary(
                resolved_auth_file,
                resolved_scope,
                "degraded",
...
def refresh_xai_oauth_auth_file(
    ...
    *,
    ...
    resolved_scope = _fallback_scope(scope)
    resolved_buffer_seconds = _resolve_buffer_seconds(buffer_seconds)
    resolved_lock_file = Path(lock_file).expanduser() if lock_file is not None else (
        resolved_auth_file.with_name(f"{resolved_auth_file.name}.lock")
    )
    ...
    try:
        ...
        if expires_at is None:
            ...
            next_refresh_check_at = observed_at + timedelta(
                seconds=max(1.0, poll_interval_seconds)
            )
        else:
            ...
            if expires_at <= observed_at:
                ...
                not access_available
                or observed_at >= expires_at - timedelta(seconds=resolved_buffer_seconds)
            ):
                credential_health = "degraded"
            else:
                credential_health = "fresh"
            usable = access_available and credential_health == "fresh"
        if expires_at is None:
...
def _fallback_scope(scope: Optional[str]) -> str:
    value = _clean_oauth_string(scope)
    return value or DEFAULT_XAI_OAUTH_SCOPE
...
def _credential_needs_refresh(
    ...
    *,
    ...
    threshold, _source, _degraded = _credential_refresh_threshold_metadata(
        credential,
        min_seconds=max(0, int(buffer_seconds)),
    )
