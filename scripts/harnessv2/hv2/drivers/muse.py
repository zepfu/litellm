"""Muse interactive driver. musela origin + inspect-derived alpha --base-url.

Dedicated tmux sessions only. Never send-keys leftover operator
``muse`` / ``musela`` / ``muselt`` / ``musel`` panes. Never aims at
:4000 / :4001. ``--base-url`` is origin-only (no ``/v1``, no
``/muse-code``).
"""
...
from hv2.musela import (
    MUSEL_WRAPPER_NAME,
    MUSELA_WRAPPER_NAME,
    MUSELT_WRAPPER_NAME,
    assert_musela_base_url,
    muse_binary,
    musela_base_url_from_resolved,
    resolve_musela_base_url,
)
...
class MuseDriver:
    def __init__(self, config: Mapping[str, Any]) -> None:
        ...
        spec = tuis.get("muse") if isinstance(tuis.get("muse"), dict) else {}
        if not spec:
            raise PlanError("tuis.muse is missing from tuis.yaml")
        self.config = config
        self.spec = spec
    ...
    def bind_resolved(self, resolved: Any) -> str:
        """Pin ``--base-url`` to the inspected alpha origin."""

        self._base_url = musela_base_url_from_resolved(resolved, self.config)
        return self._base_url

    def base_url(self) -> str:
        if self._base_url:
            return assert_musela_base_url(self._base_url, self.config)
        return resolve_musela_base_url(self.config)

    def _context(self, extra: Mapping[str, str] | None = None) -> dict[str, str]:
        lanes = (
            self.spec.get("lanes") if isinstance(self.spec.get("lanes"), dict) else {}
        )
        ctx = {
            "home": str(Path.home()),
            "session_dir": str(
                self.spec.get("session_dir") or "/tmp/hv2-muse-sessions"
            ),
            "cwd": str(self.spec.get("cwd") or "/tmp/hv2-muse-workspace"),
            "lane": str(lanes.get("alias") or "litellm-alpha"),
            "model": "",
            "selector": "",
            "pattern": "",
            "wrapper": MUSELA_WRAPPER_NAME,
    ...
    def expand_argv(
        self, key: str, extra: Mapping[str, str] | None = None
    ) -> list[str]:
        raw = as_str_list(self.spec.get(key))
        ctx = self._context(extra)
        argv = [expand_string(token, ctx) for token in raw]
        if argv and Path(argv[0]).name in ({"muse"} | _FORBIDDEN_WRAPPER_NAMES):
            argv[0] = muse_binary()
    ...
    def assert_no_print_flags(self, argv: Sequence[str]) -> None:
        ...
        for flag in (*self.forbid_flags, *self.forbid_tokens):
            if flag and flag in tokens:
                raise PlanError(
                    f"Muse argv contains forbidden token {flag!r}; "
                    "harness v2 must not use muselt/musel or leftover operator panes"
                )
        for token in tokens:
            name = Path(token).name
            if name in {MUSELT_WRAPPER_NAME, MUSEL_WRAPPER_NAME}:
                raise PlanError(
                    "Muse argv must not invoke muselt/musel; musela targets "
                    "litellm-alpha only"
                )
            if name == MUSELA_WRAPPER_NAME:
                raise PlanError(
                    "Muse argv must not exec the musela wrapper; use the muse "
                    "binary plus inspect-derived --base-url"
                )
        lowered = joined.lower()
        for token in (":4000", ":4001", "aawm-litellm", "litellm-dev", "muselt"):
            if token in lowered:
                raise ProtectedTargetError(
                    f"Muse argv refuses {token}: musela never targets "
                    "aawm-litellm / litellm-dev / :4000 / :4001 / muselt / musel"
                )
        if " --base-url " in f" {joined} " or tokens[:1] and "--base-url" in tokens:
            index = tokens.index("--base-url")
            if index + 1 >= len(tokens):
                raise PlanError("muse --base-url is missing its URL argument")
            assert_musela_base_url(tokens[index + 1], self.config)

    def child_env(self, extra: Mapping[str, str] | None = None) -> dict[str, str]:
        env_spec = (
            self.spec.get("env") if isinstance(self.spec.get("env"), dict) else {}
        )
        ctx = self._context()
        overlay = {
            str(key): expand_string(str(value), ctx) for key, value in env_spec.items()
        }
        overlay.setdefault("MUSE_DISABLE_ULTRA_ANIMATIONS", "1")
        overlay.setdefault(
            "XDG_DATA_HOME",
            str(self.spec.get("session_dir") or "/tmp/hv2-muse-sessions"),
        )
        if extra:
        ...
        for key, value in list(env.items()):
            ...
            for token in (":4000", ":4001", "aawm-litellm", "litellm-dev", "muselt"):
                if token in lowered:
                    raise ProtectedTargetError(
                        f"Muse child env {key} refuses {token}: never inherit "
                        "muselt/musel / :4000 / :4001 / aawm-litellm / litellm-dev"
                    )
        return env
    ...
    def launch_argv(self, model: str, *, lane: str | None = None) -> list[str]:
        ...
        url = self.base_url()
        if "--base-url" in argv:
            index = argv.index("--base-url")
            if index + 1 >= len(argv):
                raise PlanError("muse --base-url is missing its URL argument")
            argv[index + 1] = url
        else:
            argv[1:1] = ["--base-url", url]
        self.assert_no_print_flags(argv)
        return argv

    def alias_session_dir(self, model: str) -> Path:
        root = str(self.spec.get("session_dir") or "/tmp/hv2-muse-sessions")
    ...
    def ensure_workspace(self) -> None:
        Path(str(self.spec.get("cwd") or "/tmp/hv2-muse-workspace")).mkdir(
            parents=True, exist_ok=True
        )
        Path(str(self.spec.get("session_dir") or "/tmp/hv2-muse-sessions")).mkdir(
            parents=True, exist_ok=True
        )
    ...
    def _tmux_bin(self) -> str:
        ...
        if tmux_bin is None:
            raise HarnessError("tmux is required for the Muse interactive driver")
        return tmux_bin
    ...
    def _default_session_name(self) -> str:
        return str(self._tmux_cfg().get("session") or "muse")

    def _session_name(self) -> str:
        return self._active_session or self._default_session_name()

    def _tmux_target(self, name: str | None = None) -> str:
        """Exact tmux session target. Dots in model ids are pane paths.

        ``=name`` is not enough: tmux still splits ``muse-spark-1.3`` as
    ...
    def _session_env_pairs(self) -> list[str]:
        ...
        for key, value in child.items():
            if key.startswith("MUSE_") or key.startswith("META_"):
                token = f"{key}={value}"
    ...
    def send_keys(self, text: str) -> dict[str, Any]:
        ...
        if session in _OPERATOR_SESSION_NAMES or session == operator:
            raise PlanError(
                "refusing to send-keys leftover operator muse/musela pane "
    ...
    def wait_for_pane(
        ...
        *,
        ...
        needles = (
            [needle] if isinstance(needle, str) else [str(item) for item in needle]
        )
        timeout = float(
            timeout_seconds
    ...
    def wait_until_idle(self, timeout_seconds: float | None = None) -> bool:
        ...
        interval = self._tmux_float("poll_interval_seconds", 1)
        idle_needles = as_str_list(select.get("idle_needles")) or ["Muse Code"]
    ...
    def ensure_session(
        ...
        *,
        ...
        """Launch a dedicated interactive musela tmux session for *model*."""

        _ = child_agents
        select = self._select_spec()
        if select.get("reuse_operator_session") is True:
            raise PlanError(
                "tuis.muse.select_model.reuse_operator_session is true; "
                "harness v2 must not send-keys leftover muse/musela panes"
            )
        self.ensure_workspace()
        data_home = self.alias_session_dir(model)
        argv = self.launch_argv(model)
        if not tools:
            argv.extend(as_str_list(self.spec.get("argv_no_tools")))
        self.assert_no_print_flags(argv)
        prefix = str(self._tmux_cfg().get("harness_session_prefix") or "hv2-muse")
        safe_model = model.replace("/", "-").replace(" ", "-").replace(".", "-")
        session = f"{prefix}-{safe_model}-{os.getpid()}"
        operator = self._default_session_name()
        if session in _OPERATOR_SESSION_NAMES or session == operator:
            raise PlanError(
                "refusing to overwrite the operator Muse session "
        ...
        tmux_args.extend(["-e", f"XDG_DATA_HOME={data_home}"])
    ...
    def close_session(self) -> None:
        ...
        if not session:
            return
        if (
            session in _OPERATOR_SESSION_NAMES
            or session == self._default_session_name()
        ):
