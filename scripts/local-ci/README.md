...
- Harness `0.0.9` adds the longer peeromega timeout and keeps `openai/gpt-oss-120b:free` as the OpenRouter hard gate. This case must catch OpenRouter Responses streams that omit `response.completed`; the adapter should still persist non-zero estimated usage/cost from streamed output plus checked-in/bundled model-price JSON.
