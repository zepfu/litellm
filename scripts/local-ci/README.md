...
- Harness `0.0.12` keeps `openai/gpt-oss-120b:free` as the OpenRouter hard gate and catches OpenRouter Responses streams that omit `response.completed`; the adapter should still persist non-zero estimated usage/cost from streamed output plus checked-in/bundled model-price JSON. If that hard gate times out and the overlapping runtime logs show the exact OpenRouter provider-unavailable signature (`503`, `provider=OpenInference`, `raw=no healthy upstream`), the harness soft-fails the timeout as upstream availability without masking local adapter/logging failures.
