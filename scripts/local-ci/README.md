...
- Current NVIDIA harness cases are `claude_adapter_nvidia_deepseek_v32` and `claude_adapter_nvidia_glm47`. They are excluded from the default suite and should be selected explicitly with `--cases` while the lane is still under active validation.
- Keep `claude_adapter_nvidia_minimax_m27` as a manual-only probe for now. Use the exact `nvidia/minimaxai/minimax-m2.7` spelling when you do, but do not treat it as a required live canary until the upstream/CLI stall behavior is understood.
