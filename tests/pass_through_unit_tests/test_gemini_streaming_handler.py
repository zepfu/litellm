...
async def test_route_streaming_logging_to_handler_accepts_explicit_response(
    mock_vertex_handler,
    mock_gemini_handler,
):
    """Verify the handler works when an httpx.Response is explicitly supplied."""
    mock_gemini_handler.return_value = {
        "result": {"response": "gemini"},
        "kwargs": {"custom_llm_provider": "gemini", "model": "gemini-3-flash-preview"},
    }
    ...
    await PassThroughStreamingHandler._route_streaming_logging_to_handler(
        litellm_logging_obj=litellm_logging_obj,
        passthrough_success_handler_obj=MagicMock(),
        url_route="https://generativelanguage.googleapis.com/v1beta:generateContent",
        request_body={"model": "gemini-3-flash-preview"},
        endpoint_type=EndpointType.VERTEX_AI,
        start_time=datetime.now(),
        raw_bytes=[b'{"candidates":[{"content":{"parts":[{"text":"hello"}]}}}'],
        end_time=datetime.now(),
