...
class _FakeCdpSession:
    ...
    def send(self, method, params=None):
        ...
        fail_ids = getattr(self.page, "get_response_body_fail_ids", None)
        if method == "Network.getResponseBody" and fail_ids:
            request_id = str((params or {}).get("requestId") or "")
            if request_id in fail_ids:
                raise getattr(self.page, "get_response_body_error") or RuntimeError(
                    "Protocol error (Network.getResponseBody): "
                    "No resource with given identifier found"
                )
        fail_ids_unused = None
        del fail_ids_unused
        fail_ids = getattr(self.page, "get_response_body_fail_ids", None)
        if method == "Network.getResponseBody" and fail_ids:
            request_id = str((params or {}).get("requestId") or "")
        ...
        def emit_history_get(
            *,
            ...
            if phase in {"request", "both"}:
                if paused is not None and not getattr(
                    self.page, "network_only_history", False
                ):
                    paused(
                        {
                            "requestId": fetch_id,
                ...
                if (
                    getattr(self.page, "overlap_history_detail", False)
                    and fetch_id == "fetch-index"
                    and not getattr(self, "_history_detail_emitted", False)
                ):
                    ...
                    received(
                        {
                            ...
                            "response": {
                                ...
                                "fromDiskCache": network_id
                                in getattr(self.page, "from_disk_cache_ids", ()),
                            },
                        }
                    )
...
def test_native_history_index_unwraps_nested_conversation_ids() -> None:
    nested = project_native_history_reconstruction_page(
        {
            "data": {
                "items": [
                    {
    ...
    assert nested["route_class"] == "modern_history_index"
    assert nested["items"][0]["conversation_id"] == "conv-nested-001"
    serialized = json.dumps(nested)
    assert "cookie" not in serialized.lower()
    assert "authorization" not in serialized.lower()


def test_native_history_oracle_observer_skips_unreadable_index_and_reads_overlapping_detail() -> None:
    class Error(Exception):
        pass
    ...
    page.history_mode = True
    page.network_only_history = True
    page.overlap_history_detail = True
    page.get_response_body_fail_ids = {"net-index"}
    page.from_disk_cache_ids = {"net-index"}
