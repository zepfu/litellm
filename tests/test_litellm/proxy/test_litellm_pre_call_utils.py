...
@pytest.mark.asyncio
async def test_anthropic_byok_promotes_forwarded_x_api_key_for_model_group():
    """Model-group header forwarding should promote client x-api-key to api_key for Anthropic."""
    from litellm.proxy.litellm_pre_call_utils import add_litellm_data_to_request
    ...
    request_mock.headers = {
        ...
        "x-api-key": "sk-ant-api03-client-byok",
        "content-type": "application/json",
    }
    request_mock.client = MagicMock()
    request_mock.client.host = "127.0.0.1"
    ...
    mock_settings = MagicMock(forward_client_headers_to_llm_api=["anthropic/*"])
    with patch("litellm.model_group_settings", mock_settings), patch(
    ...
    assert updated["api_key"] == "sk-ant-api03-client-byok"
    assert updated["headers"]["x-api-key"] == "sk-ant-api03-client-byok"


@pytest.mark.asyncio
async def test_proxy_auth_headers_not_promoted_to_anthropic_api_key():
    """LiteLLM proxy auth must not be used as Anthropic provider api_key."""
    from litellm.proxy.litellm_pre_call_utils import add_litellm_data_to_request
    ...
    request_mock.query_params = {}
    request_mock.headers = {
        "x-litellm-api-key": "sk-litellm-proxy-only",
        "content-type": "application/json",
    }
    request_mock.client = MagicMock()
    request_mock.client.host = "127.0.0.1"
...
@pytest.mark.parametrize(
    ...
    ids=["prefixed_anthropic_model", "bare_direct_anthropic_model"],
)
@pytest.mark.asyncio
async def test_anthropic_byok_promotes_x_api_key_when_auth_is_authorization_without_model_group(
    model: str,
):
    """Provider x-api-key should promote to api_key when proxy auth used Authorization only (D1-482)."""
    from litellm.proxy.litellm_pre_call_utils import add_litellm_data_to_request
    ...
    with patch("litellm.model_group_settings", None), patch(
        "litellm.proxy.proxy_server.llm_router", None
    ), patch("litellm.proxy.proxy_server.premium_user", True):
        updated = await add_litellm_data_to_request(
            data=data,
    ...
    assert updated["api_key"] == "sk-ant-api03-client-byok"
    assert updated["api_key"] != "Bearer sk-proxy-key"
    assert updated["api_key"] != "sk-proxy-key"



@pytest.mark.asyncio
async def test_anthropic_oauth_promotes_authorization_bearer_sk_ant_oat_for_direct_model():
    """Authorization Bearer sk-ant-oat* should promote to api_key for direct Anthropic models."""
    from litellm.proxy.litellm_pre_call_utils import add_litellm_data_to_request

    oauth_token = "sk-ant-oat01-fake-token-for-testing-123456789abcdef"
    request_mock = MagicMock(spec=Request)
    request_mock.url.path = "/v1/chat/completions"
    request_mock.url = MagicMock()
    request_mock.url.__str__.return_value = "http://localhost/v1/chat/completions"
    ...
    assert updated["api_key"] == oauth_token
    assert "headers" not in updated or all(
        header.lower() != "x-api-key" for header in updated["headers"]
    )


@pytest.mark.asyncio
async def test_anthropic_oauth_promotes_authorization_bearer_for_model_group_forwarding():
    """Model-group forwarding should also promote Authorization Bearer sk-ant-oat*."""
    from litellm.proxy.litellm_pre_call_utils import add_litellm_data_to_request
...
@pytest.mark.asyncio
async def test_proxy_authorization_bearer_not_promoted_to_anthropic_api_key():
    """Non-OAuth proxy Authorization bearer must not be promoted to provider api_key."""
    from litellm.proxy.litellm_pre_call_utils import add_litellm_data_to_request
...
@pytest.mark.asyncio
async def test_anthropic_oauth_promotes_with_global_forward_client_headers_to_llm_api():
    """Global forward_client_headers_to_llm_api should promote OAuth Bearer sk-ant-oat* (D1-482)."""
    from litellm.proxy.litellm_pre_call_utils import add_litellm_data_to_request
    ...
    assert updated["api_key"] == oauth_token
    assert "headers" not in updated or all(
        header.lower() != "x-api-key" for header in updated.get("headers", {})
    )


@pytest.mark.asyncio
async def test_proxy_authorization_bearer_not_promoted_with_global_forward_client_headers():
    """Non-OAuth proxy Authorization must not promote when only global header forwarding is on."""
    from litellm.proxy.litellm_pre_call_utils import add_litellm_data_to_request
...
@pytest.mark.asyncio
async def test_non_anthropic_route_does_not_promote_x_api_key_to_api_key():
    """forward_llm_provider_auth_headers must not promote x-api-key for non-Anthropic models."""
    from litellm.proxy.litellm_pre_call_utils import add_litellm_data_to_request
    ...
    request_mock.headers = {
        "authorization": "Bearer sk-proxy-key",
        "x-api-key": "sk-ant-api03-should-not-apply",
        "content-type": "application/json",
    }
    request_mock.client = MagicMock()
    request_mock.client.host = "127.0.0.1"
