...
class TestProxySettingEndpoints:
    ...
    def test_ui_settings_model_allowlists_disable_custom_api_keys(self):
        """UISettings and allowlist stay aligned for disable_custom_api_keys."""
        from litellm.proxy.ui_crud_endpoints.proxy_setting_endpoints import (
            ALLOWED_UI_SETTINGS_FIELDS,
            UISettings,
        )

        assert "disable_custom_api_keys" in UISettings.model_fields
        assert "disable_custom_api_keys" in ALLOWED_UI_SETTINGS_FIELDS
        assert set(UISettings.model_fields.keys()) == ALLOWED_UI_SETTINGS_FIELDS

        defaults = UISettings()
        assert defaults.disable_custom_api_keys is False
        assert UISettings(disable_custom_api_keys=True).model_dump(
            exclude_unset=True
        ) == {"disable_custom_api_keys": True}

    def test_get_ui_settings_includes_disable_custom_api_keys_schema(
        self, mock_auth, monkeypatch
    ):
        """Upstream-equivalent UI setting must appear in field_schema and values."""
        import asyncio
        from unittest.mock import AsyncMock, MagicMock
        ...
        data = asyncio.run(get_ui_settings())
        if hasattr(data, "model_dump"):
            data = data.model_dump()
        elif not isinstance(data, dict):
            data = dict(data)

        assert data["values"]["disable_custom_api_keys"] is True
        assert "unexpected_flag" not in data["values"]
        assert "disable_custom_api_keys" in data["field_schema"]["properties"]
        prop = data["field_schema"]["properties"]["disable_custom_api_keys"]
        assert prop.get("type") == "boolean"
        assert "custom api key" in prop.get("description", "").lower()

    def test_update_ui_settings_disable_custom_api_keys(self, mock_auth, monkeypatch):
        """Admins can persist disable_custom_api_keys via update_ui_settings."""
        import asyncio
        from unittest.mock import AsyncMock, MagicMock
        ...
        settings = UISettings(disable_custom_api_keys=True)
        data = asyncio.run(
            update_ui_settings(settings=settings, user_api_key_dict=mock_user_auth)
        )

        assert data["status"] == "success"
        assert data["settings"]["disable_custom_api_keys"] is True
        assert "unexpected_flag" not in data["settings"]

        assert mock_prisma.db.litellm_uisettings.upsert.called
        call_args = mock_prisma.db.litellm_uisettings.upsert.call_args
        create_data = call_args.kwargs["data"]["create"]
        stored_settings = json.loads(create_data["ui_settings"])
        assert stored_settings["disable_custom_api_keys"] is True

    def test_update_ui_settings_strips_legacy_unknown_keys(
        self, mock_auth, monkeypatch
    ):
        """PATCH re-sanitizes existing DB rows so legacy junk is not re-persisted."""
        import asyncio
        from unittest.mock import AsyncMock, MagicMock
        ...
        assert data["settings"]["disable_custom_api_keys"] is True
        assert data["settings"]["disable_model_add_for_internal_users"] is True
        assert "legacy_junk" not in data["settings"]

        stored_settings = json.loads(
            mock_prisma.db.litellm_uisettings.upsert.call_args.kwargs["data"]["update"][
                "ui_settings"
            ]
        )
    ...
    def test_get_ui_settings_cached_exposes_disable_custom_api_keys(
        self, mock_auth, monkeypatch
    ):
        """get_ui_settings_cached returns allowlisted disable_custom_api_keys."""
        import asyncio
        from unittest.mock import AsyncMock, MagicMock
        ...
        result = asyncio.run(get_ui_settings_cached())
        assert result["disable_custom_api_keys"] is True
        assert "unsupported_flag" not in result
        mock_cache.async_set_cache.assert_awaited()
        set_kwargs = mock_cache.async_set_cache.await_args.kwargs
        assert set_kwargs["key"] == UI_SETTINGS_CACHE_KEY
        assert set_kwargs["value"]["disable_custom_api_keys"] is True
