...
def test_get_end_user_id_uses_later_customer_header_when_earlier_absent():
    """Multi-header mapping must honor non-first CUSTOMER headers present on the request."""
    from litellm.proxy.auth.auth_utils import get_end_user_id_from_request_body

    mappings = [
        {"header_name": "x-primary-customer", "litellm_user_role": "customer"},
        {"header_name": "x-secondary-customer", "litellm_user_role": "customer"},
    ]
    general_settings = {"user_header_mappings": mappings}
    headers = {"X-Secondary-Customer": "secondary-user-99"}
    ...
    assert result == "secondary-user-99"
